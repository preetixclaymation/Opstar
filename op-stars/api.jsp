...
private Response<Map<String, CountData>> getCounts(String email, String[] movieIds) {
  ...
  for(String movieId : movieIds){
      System.out.println("THESE ARE MOVIE ID"+movieId);
  }
  try {
...
private Map<String, CountData> likeDislikeCount(String email, String[] movieIds) throws Exception {
  ...
  try (Connection con = DriverManager.getConnection(DB_URL);
    ...
    if(rs.next())
        System.out.println("Recordcount: "+ rs.getString("movieid"));
    while (rs.next()) {
      String movieId = rs.getString("movieid");
      System.out.println("MOVIE ID FROM DB"+movieId);
...
private Response<?> addComment(String email, String comment, String movieId) {
  ...
  System.out.println("ADDED NEW COMMENT ON VIDEO"+movieId);
...
private Response<Boolean> addConfig(Boolean isAdmin, String email, ServletContext application, String type, String text, 
  ...
  try  {
      ...
      String filePath = basePath + "/cards/config.json";
...
private Response<Boolean> deleteConfig(Boolean isAdmin, String email, ServletContext application, String movieIdToDelete) throws Exception {
  ...
  try  {
      ...
      if (deleted) {
	      String filePath = application.getRealPath("/") + "/cards/config.json";
	      String json = new GsonBuilder().setPrettyPrinting().create().toJson(config);
	      try (FileWriter fw = new FileWriter(filePath)) {
	        fw.write(json);
	      }
	      deleteMovieData(movieIdToDelete);
      }
      res.data = deleted;
      res.status = 200;
      res.message = "Success";
...
private void deleteMovieData(String movieId) throws Exception {
  try (Connection con = DriverManager.getConnection(DB_URL)) {
	  String query = "delete from user_vote where movieid = ?";
	  try (PreparedStatement stmt = con.prepareStatement(query)) {
	    
	    stmt.setString(1, movieId);
	    stmt.executeUpdate();
	  }
	  query = "delete from user_comment where movieid = ?";
	  try (PreparedStatement stmt = con.prepareStatement(query)) {
...
private List<Config> readConfig(ServletContext application) throws Exception {
  try (InputStream inputStream = new FileInputStream(application.getRealPath("/") + "/cards/config.json")) {
